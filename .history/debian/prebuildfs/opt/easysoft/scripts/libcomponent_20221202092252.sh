...
ZENDAS_URL="https://dl.cnezsoft.com/zendas/1.2.stable/ZenDAS.1.2.stable.zip"
...
z_download() {
    ...
    local zdas_base_name="ZenDAS.1.2.stable.zip"
    ...
    "zdoo")
        ...
        ;;
    "zendas")
        wget --no-check-certificate --quiet --output-document=/tmp/"${1}" "${ZENDAS_URL}/${version}/${zdoo_base_name}"
        unzip -qq -d ${directory} /tmp/"${1}" && rm -rf /apps/zdoo/www/data
        rm /tmp/"${1}"
        ;;
    *)
